...
- [ ] **接上即時 API(即使是 dry-run)的唯一剩餘阻塞項:需要專案負責人申請一組 Binance Spot Testnet 沙盒金鑰**(`testnet.binance.vision`,GitHub 登入即可,無需 KYC),AI 助理無法代為完成這一步(需要互動式 OAuth 登入)。**已用真實網路實測確認**:即使是沙盒,`reload_markets()` 仍需要有效 API key 才能啟動——金鑰是硬性必要條件,不是選配,但現在需要的金鑰申請門檻遠低於正式帳戶。金鑰到位後複製 `user_data/configs/secrets-testnet.example.json` 為 `secrets-testnet.json` 並填入,即可用 `scripts/preflight_check.py` 啟動,不需要再回頭補任何程式碼。
