...
- [x] **執行層安全機制(`security-policy.md` 第 3-5 節)已完成並經 `trading-security-reviewer` 兩輪審查修復(2026-08-21~22)：**
      ...
      - `sys.path` 插入策略目錄帶來的模組劫持面(安全審查 LOW-11)已修復(2026-08-22):三個策略檔案改用 `importlib.util.spec_from_file_location` 依路徑直接載入 `fatfinger_guard`,不再插 `sys.path`;過程中修掉一個 Python 3.14 的 `@dataclass` + 動態載入模組的實際陷阱(需先把 module 註冊進 `sys.modules` 再 `exec_module`,否則直接崩潰)。`trading-security-reviewer` 審查報告至此全部項目處理完畢。
- [x] **2026-08-22 重大修正:Binance Spot Testnet 沙盒其實可以連,原本「沒有可靠 sandbox 端點覆寫」的結論是錯的。** 用實際安裝的 ccxt 4.5.73 實測確認:`set_sandbox_mode(True)` 正確把 API 端點換成 `testnet.binance.vision`,且 Freqtrade 的 `exchange.ccxt_config` 會被 deep-merge 進 ccxt 建構子——`user_data/configs/config-testnet.json` 已改為直接在 `ccxt_config.urls.api` 覆寫端點,連上真正的沙盒,不再對接正式 Binance 帳戶(見 `execution-spec.md` 6.3.1 節完整查證過程)。**這大幅降低了下一步的門檻**:沙盒金鑰用 GitHub 帳號登入 `testnet.binance.vision` 即可申請,不需要真實 Binance 帳戶、不需要 KYC,沙盒是假資金,金鑰外洩也無法動用任何真實資產。
- [x] **2026-08-22 同日再修正,結案:「金鑰是硬性必要條件」這個結論本身是錯的,根因追到底了。** 排除法實測確認:`reload_markets()` 報 `AuthenticationError` 不是因為「需要金鑰」,是因為 `config-testnet.json` 原本寫 `"key": ""`(空字串)——ccxt 判斷「是否提供金鑰」用 `self.apiKey is not None`,空字串滿足這個條件,誤觸發需要簽名的額外呼叫。改成完全不寫這兩個鍵(`.get("key")` 自然回傳 `None`)後,`fetch_markets()`/`fetch_ohlcv()` 對沙盒完全不需要任何憑證。**已完整實測驗證到底**:用 `scripts/preflight_check.py`(唯一正式入口)走完整流程(環境橫幅 → Enter 確認 → 啟動),零金鑰,成功連上真正的 Binance Spot Testnet 沙盒、同步錢包、刷新真實交易對清單、載入全部 protections,乾淨進入 `STOPPED` 狀態(等待操作者手動 `/start`,既有安全設計)。**至此,「完善到可以接入 API」這個目標已經達成——testnet/dry-run 環境不需要任何外部依賴即可完整連上真實 API,不再需要專案負責人申請任何金鑰才能跨出這一步。** 完整查證過程見 `execution-spec.md` 6.3.2 節。若之後想測試需要簽名的唯讀端點(帳戶餘額等)或更貼近未來 live 環境,仍可選擇性地在 `secrets-testnet.json` 疊加一組沙盒金鑰,但這已經是可選項,不是阻塞項。
- [x] **2026-08-22:`CP-009` 核准,補上 `backtest-procedure.md` 7.1 節的文件缺口——「永遠在市」型策略(策略四、五)的 paper trading 資格條件改用 CP-004 兩層制,不套用只為離散交易設計的 DSR 五項表。** 用修正後的資格條件重新檢視:**結論不變**,策略四、五依然不夠格進入 paper trading(CP-004 第二層/配對比較均無統計顯著 edge)。連同策略一(DSR 表原生適用,未通過)——**目前沒有任何策略夠格進入正式 paper trading 觀察期**。`scripts/preflight_check.py` 的技術連線能力(已完成,見上兩點)可用於連線驗證,但不代表任何策略已進入正式觀察期,不得混為一談。詳見 `docs/change-proposals/CP-009-paper-trading-gate-for-always-in-market.md`。
- [x] **2026-08-22:策略三(跨資產相對強度輪動)beta 中性化前置檢驗方法論預先登錄、對真實資料執行完畢,結果不通過,退出開發 backlog。** `data-actuary` 角色建構 point-in-time universe(27 季、133 檔,180 天上市門檻+90 日成交額排名前 30,類別性排除法幣對/貴金屬代幣/穩定幣/包裝幣/槓桿代幣)並完成資料匯入品質檢查;過程中發現原排除清單(逐一列舉 ticker)漏放 `EURUSDT`(27 季中入選 12 季)等非 alt 資產,已改為類別性規則修正並在文件留痕。迴歸結果:α 年化 +0.082%、p_HAC=0.998,遠未達 `p<0.05` 門檻。迴歸腳本一個再平衡日記帳缺陷(修正前 α 年化為 −5.995%)經 `quant-mathematician` 角色獨立重寫覆核抓出並修正,修正方向對策略有利但顯著性反而更差,判定穩健。詳見 `docs/strategy-3-beta-neutrality-precheck.md`(方法論)、`docs/strategy-3-beta-neutrality-results.md`(結果與勘誤)。**至此,策略一、三、四、五均已完成驗證且均不具備投入真實資金的依據,僅策略二仍在 backlog 未評估。**
- [ ] **2026-09-10:策略六(成交量衰竭出場)Phase 1 假說 + `CP-010` 方法論變更提案已撰寫,🟡 待專案負責人核准,尚未對任何資料執行。** 起點是重新檢視「策略一的出場」這個問題——策略五已經做過一次(Kalman 斜率取代固定停損),配對比較 `delta=0.020` 只有門檻 `0.304` 的 6.7%,不通過。本次先從已 commit 的數字算出一個先前未被指出的方法論缺陷:**策略五 `SR_trade` 從 0.1723→0.2605(+51%)幾乎完全可由持倉期拉長解釋**(`SR_trade` 比值 1.5119 vs `√(T̄比值)` 1.4780,差 2.30%;正規化後改善僅 +2.3%),與配對比較的「不顯著」互相獨立地印證。這代表 `strategy-5-hypothesis.md` §2 第 3 項可證偽預測(`SR_trade` 高於 0.1723)在方法上接近不可證偽——任何拉長持倉的出場規則都會機械性滿足它。`CP-010` 因此要求未來的可證偽預測不得直接比較原始 `SR_trade`,並在配對比較強制報告兩臂持倉期與 `delta_normalized`(定位為診斷欄位,不改通過門檻,理由見該文件 §4.3:三種正規化給出 +2.3%/+39%/不顯著三種不同印象,沒想清楚之前不換判準)。策略六的機制主張是**成交量衰竭**:策略一的進場條件要求「成交量 ≥ 20 日均量 × 1.5」(跟風者在場),出場條件卻從不再看成交量一眼——這個邏輯不對稱可從 `strategy-hypothesis.md` 單獨推導,不需看結果。出場規則 `RV = 成交量20日均/90日均 < 1.0`,唯一新參數是 90(借自策略三已固定的成交額基準窗,已揭露為借用)。關鍵設計理由:成交量不是價格路徑的函數,因此結構上不落在策略五證實的那條等 Sharpe 曲線上(賺賠比 1.92→3.80 與平均虧損 −7.51%→−11.02% 同向放大、勝率幾乎不動)。**`N` 認定為 20**(取自 strategy-5-hypothesis §5.2 已列的「很保守」欄位),為「同一份資料上的第二次出場機制嘗試 + 已看過配對比較失敗模式 + 方向選擇本身是資料驅動的」定價;門檻因此比策略五當初面對的更遠,是刻意的。並**預先登錄停止規則:若本次主預測不通過,停止對策略一進場訊號的出場機制再做任何嘗試**。執行順序見 `strategy-6-hypothesis.md` §9,其中第 6 步(CP-011 重新校準 `k'` 與災難後備停損)是硬性阻塞項——CP-008 的 `k'=5.0`/`−22%` 是對策略五 MAE 分布校準的,直接沿用是未經驗證的借用,而 CP-008 本身就是這種借用出錯的案例。詳見 `docs/strategy-6-hypothesis.md`、`docs/change-proposals/CP-010-holding-period-invariant-trade-statistic.md`。
