...
> ### ⚠️ 1.2 / 1.3 節的資料範圍已被實際取得的資料取代(2026-08-08)
>
> 本節原訂 2019-09 下載、2020-01 起算,是在**尚未取得任何資料**時依「幣安流動性足夠的年份」推估的。實際取得的資料範圍是 **2017-08-17 ~ 2026-07-31,共 3,271 根日 K(約 8.96 年)**,兩個交易對相同,詳見 [`data-requirements.md`](./data-requirements.md)。
>
> 起點提前的理由不是「資料多一點比較好」,而是 [`feasibility-paths.md`](./feasibility-paths.md) 第 2 節算出的具體差額:在 `N=5` 之下,6.6 年的通過門檻是 1.37、8.96 年是 1.18。**多出來的 2.4 年把 `σ_SR` 估計誤差的容錯從「高估 20% 就失守」推到「高估 50% 仍守得住」。**
>
> ⚠️ 代價是納入 2017 泡沫與 2018 熊市,這兩段的市場結構與現在差異極大,**同質性假設存疑**。這是一個有意識接受的取捨,記錄在 [`feasibility-paths.md`](./feasibility-paths.md) 3.3 節。
>
> 1.4 節的品質規則**完全不受影響**,且已於 2026-08-08 對這份資料實際執行完畢(缺漏 0、疑似異常 0),並已程式化為 `analysis/data_quality.py`。
...
> ## 🔴 本節 4.1–4.4 已被 [`CP-003`](./change-proposals/CP-003-fixed-parameters.md) 取代(2026-08-08 核准)
>
> 以下 4.1–4.4 節的內容**保留作為變更紀錄**,不再是要執行的流程。實際執行依本區塊。
>
> ### 4.0 CP-003 之後的驗證流程
>
> **變更的核心:** 參數不再從資料中搜出來。5 個策略參數固定 4 個,只對 `donchian_period` 掃描 {20, 30, 40, 50, 55} 這 5 個點,使 `N=5`(原為 200)。
>
> **這讓通過門檻從年化 Sharpe 4.01 降到 1.18**,首次落入 [`scope.md`](./scope.md) 的 1.0–1.5 目標區間。代價是**真的放棄了搜尋 195 組參數的能力**,不是把標準調低 —— 完整論證見 CP-003 5.1 節。
>
> #### 執行步驟
>
> | 步驟 | 動作 | 產出 |
> |---|---|---|
> | 1 | `freqtrade lookahead-analysis` 前置檢查(4.2 節**仍然適用**,未被取代) | 策略程式碼無 lookahead 的確認 |
> | 2 | 對 5 個 `donchian_period` 各跑一次全樣本 backtest(2017-08-17 ~ 2026-07-31) | 5 組交易明細 + 5 個 `SR_trade` |
> | 3 | `analysis/cost_model.py` 套用進場 +5bps / 出場 −5bps(2.2 節不變) | 成本調整後的交易明細 |
> | 4 | 從 5 個 `SR_trade` 算樣本標準差 → `σ_SR` | DSR 公式的輸入 |
> | 5 | `analysis/sample_size.py` 算 `n_eff`、`IF`(方法不變) | `n_eff`(硬性下限 30 仍適用) |
> | 6 | `analysis/significance.py` 算 `SR0`(`N=5`)與 DSR | **主檢定結果** |
> | 7 | `analysis/drawdown_mc.py` 蒙地卡羅回撤(第 6 節不變) | `P(MDD>15%)`、`P(MDD>20%)` |
> | 8 | 分 fold 診斷(見下方 4.0.2) | regime 穩健性 |
>
> #### 4.0.1 為什麼用 5 次明確 backtest 取代 hyperopt
>
> 一維空間跑 TPE 沒有意義,5 個點直接窮舉即可;而且完全確定性,沒有 `--random-state` 的重現性疑慮。**`σ_SR` 的計算基礎不變** —— 原設計要的就是各組參數的 `SR_trade` 分布,5 次 backtest 直接給出 5 個值。
>
> 原 4.3 節步驟 1 的自訂 `PurgedTradeSharpeLoss` 因此**不再需要**:它存在的唯一理由是讓 purge 規則在 hyperopt 的每次評估中生效,而現在沒有 hyperopt。
>
> #### 4.0.2 walk-forward 保留,但角色改變
>
> | | 原角色 | **新角色** |
> |---|---|---|
> | IS/OOS 切分 | 選參數 / 驗證 | **不再需要** —— 整段 8.96 年是同一個樣本 |
> | 分 fold 統計 | 主檢定來源 | **regime 穩健性診斷** —— 4.4 節「≥70% fold Sharpe > 0」檢查仍執行 |
> | purge / embargo | 防 IS→OOS 洩漏 | **仍需要** —— 用於分 fold 診斷的邊界處理 |
>
> **最容易被誤解的一點:** 整段資料變成合法樣本,**不是因為放寬了什麼**,而是因為參數不再從資料搜出來,原本要用 IS/OOS 隔離的那個洩漏管道**不存在了**。DSR 的 `N=5` 已完整計入「我們試了 5 次」。
>
> #### 4.0.3 Pass A / Pass B 兩輪執行不再需要
>
> 4.1 節的兩輪設計是為了校準 `embargo_days`,而 embargo 原本的作用是防止 IS→OOS 洩漏。既然不再有 IS/OOS 切分,**校準的對象消失了**。分 fold 診斷仍用 30 天的 embargo 下限,但那是診斷用的邊界處理,不影響主檢定,不需要為它跑一輪完整流程。
>
> #### 4.0.4 ⛔ 一條不可逾越的線
>
> **若這 5 個點跑出來都不通過,正確的反應是接受策略未通過,不得回頭加點、不得改動那 4 個固定值。** 加點會讓 `N` 變大、門檻上升,而且不會有任何機制報錯 —— 唯一的防線是紀律與 CP-003 核准 commit(`8be1d9e`)的時間戳。
